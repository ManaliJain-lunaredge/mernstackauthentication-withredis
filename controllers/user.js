...
import { revokeRefreshToken } from "../middlewares/isAuth.js";
export const register = TryCatch(async (req, res) => {
    ...
    if (!validation.success) {
        ...
        if (zodError?.success && Array.isArray(zodError.issues)) {
            ...
            firstErrorMessage = allErrors[0]?.message || "Validation Errors"
        }
        return res.status(400).json({
            message: firstErrorMessage, error: allErrors
        })
    }
    ...
    const rateLimitKey = `request-rate-limit:${req.ip}:${email}`;
...
export const loginUser = TryCatch(async (req, res) => {
    ...
    await redisClient.set(rateLimitKey, "true", { EX: 60 })
    res.json({
        message: "If your email is valid then opt has been sent, It will be valid for 5 min"
    })
...
export const verifyOtp = TryCatch(async (req, res) => {
    ...
    if (!email || !otp) {
        return res.status(400).json({
            message: "Please provide aall details"
        })
    }
